...
class Server:
    ...
    def push(self, cID, departure_time):
    ...
    def check_carrier(self, customer_list):
        flag = 0
...
def timing_routine(tmp_customer, customer_queue, server_list, curr_time, total_server, termination_time):
    ...
    if not min_queue_time == termination_time * 100:
    ...
    if min_queue_time == termination_time * 100:
        min_server_time = termination_time * 100
        for item in server_list:
            for time in item.departure_time_list:
                if time < min_server_time:
...
def call_event(customer_list, tmp_customer, customer_queue, server_list, curr_time, total_server, record, day, dining, finish):
        ...
        if not item.idle:
            ...
            for time in item.departure_time_list:
                if time <= curr_time:
                    dining = dining - 1
                    finish = finish + 1
                    clock_time = str(math.floor(time/60)+11)+":"+str(time%60)
    ...
    table = int(np.random.uniform(total_server))
    ...
    for item in customer_list:
        if item.carrier == 1:
            if not item.arrival_time == -1:
                #print(item.ID)
                if item.period  >= incubation_period:
...
N = 100
T_size = 36
T_detail = [2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2]
p = 0.8
incubation_period = 3
k = 2
customer_list = []
